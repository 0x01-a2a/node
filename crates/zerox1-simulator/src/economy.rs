...
fn median_profit_at_stake(
    ...
    let mut profits: Vec<f64> = trials.iter().map(|t| {
        // Re-derive slash total at the new stake level.
        let slash_total = t.detection_epochs.iter()
            .filter(|e| e.is_some())
            .count() as f64 * stake * params.slash_rate;
        t.total_premium_earned - slash_total
    }).collect();

    if profits.is_empty() { return 0.0; }
...
pub fn derive_betas(
    ...
    let b1 = if total > 0.0 { entropy_contribution / total } else { 0.85 };
    let b2 = 1.0 - b1;

    BetaCoefficients { b1, b2, b3: 0.0, b4: 0.0 }
}
...
pub fn aggregate(trials: &[TrialResult]) -> AggregateStats {
    ...
    let mut all_epochs: Vec<usize> = trials.iter()
