...
pub fn print_report(
    ...
    println!();
    println!(
        "  Network     : {} agents  ({} cartel, {} honest)",
        params.n_agents,
        n_cartel,
        params.n_agents - n_cartel
    );
    println!("  Cartel type : {}", params.cartel_type);
    println!("  Epochs      : {}   Trials: {}", params.n_epochs, n_trials);
    println!(
        "  Base stake  : ${:.0} USDC    Rep premium: ${:.2}/epoch",
        params.base_stake, params.rep_premium
    );
    println!(
        "  Anomaly τ   : {:.2}   Slash rate: {:.0}%",
        params.anomaly_threshold,
        params.slash_rate * 100.0
    );
    println!();
    println!("── Detection ─────────────────────────────────────────────────");
    println!(
        "  True positive rate  : {:.1}%  (cartel agents correctly flagged)",
        stats.mean_detection_rate * 100.0
    );
    println!(
        "  False positive rate : {:.1}%  (honest agents wrongly flagged)",
    ...
    println!("── Economics ─────────────────────────────────────────────────");
    println!(
        "  Mean net profit     : {:+.2} USDC  ({:.0}% of trials profitable)",
        stats.mean_net_profit,
        stats.pct_profitable_trials * 100.0
    );
    println!("  α_min (derived)     : ${alpha_min:.0} USDC");
    println!(
        "  Recommended stake   : ${:.0} USDC  (α_min + 20% safety margin)",
        alpha_min * 1.20
    );
    println!();
    ...
    println!(
        "  β₃ (coordination C) : {:.3}  [GAP-02: pending ownership clustering]",
        betas.b3
    );
    println!(
        "  β₄ (systemic S)     : {:.3}  [GAP-06: pending SRI circuit breaker]",
        betas.b4
    );
    println!();
    println!("  Stake multiplier formula:");
    println!(
        "    Sv = Sv_base × (1 + {:.3}·A + {:.3}·R + {:.3}·C + {:.3}·S)",
    ...
    if stats.pct_profitable_trials > 0.10 {
        println!(
            "  ⚠  Attack profitable in >{:.0}% of trials — α_min may need review.",
            stats.pct_profitable_trials * 100.0
        );
    }
    println!();
