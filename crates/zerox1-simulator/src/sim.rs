...
impl std::fmt::Display for CartelType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::C1 => write!(f, "C-1 (static hugging)"),
            Self::C2 => write!(f, "C-2 (rotational evasion)"),
        }
    }
}
...
impl TrialResult {
    ...
    pub fn median_detection_epoch(&self) -> Option<f64> {
        let mut epochs: Vec<usize> = self.detection_epochs.iter().filter_map(|&e| e).collect();
        if epochs.is_empty() {
            return None;
        }
...
pub fn run_trial<R: Rng>(params: &SimParams, rng: &mut R) -> TrialResult {
    ...
    let all_ids: Vec<[u8; 32]> = honest_agents
        .iter()
